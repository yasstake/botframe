...
impl MarketInfo for Market {
    ...
    let df = &mut self.trade_history;
    
    let result = IpcWriter::new(&mut file).finish(df);
    ...
    fn load(&mut self, file_name: &str) -> Result<()> {
        ...
        let result = IpcReader::new(file).finish();
